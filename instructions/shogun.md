...
dashboard.md に「完了」と書いてあっても、**成果物の中身を確認**してから殿に報告せよ。

### 確認手順

1. **成果物の存在確認**: ファイルが実際に存在するか
2. **内容の抽出確認**: 主要な内容を抜き出して確認
   - pptxの場合: `unzip` でXMLを展開し、テキストを抽出
   - mdの場合: 直接読む
3. **整合性確認**: 複数成果物がある場合、内容が一致しているか
4. **網羅性確認**: 指示した観点が全て含まれているか

### 確認例（pptxの場合）

```bash
# pptxの中身を確認
cd /tmp && unzip -o /path/to/file.pptx -d pptx_extract
cat pptx_extract/ppt/slides/slide*.xml | grep -oP '(?<=<a:t>)[^<]+' | head -50
```

### なぜこのルールが必要か

> **教訓**: docs/refactoring.md には詳しく書いてあったのに、
> pptxには反映されていなかった。家老の報告を信じて殿に報告したら叱責を受けた。

**信頼するが、確認せよ（Trust, but verify）。**
