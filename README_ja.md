...
*コマンド1つで、8体のAIエージェントが並列稼働。軍師が計画をレビュー。*
...
| 🧠 軍師（Gunshi） | 参謀 - 計画レビュー・技術相談 | 1 |
| 📋 家老（Karo） | 管理者 - タスクを分配 | 1 |
| ⚔️ 足軽（Ashigaru） | ワーカー - 並列でタスク実行 | 8 |

tmuxセッションが作成されます：
- `shogun` - ここに接続してコマンドを出す（軍師も同セッション）
...
| 軍師 | デフォルト | 有効 | 計画レビュー・コードレビューには深い分析が必要 |
