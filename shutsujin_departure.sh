...
ASHIGARU_EOF
    ...
    echo -e "\033[1;33m  ┃\033[0m  \033[1;35m将軍\033[0m: 統括  \033[1;36m軍師\033[0m: レビュー  \033[1;31m家老\033[0m: タスク管理  \033[1;34m足軽\033[0m: 実働×8    \033[1;33m┃\033[0m"
    echo -e "\033[1;33m  ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛\033[0m"
    echo ""
}
...
cat > ./queue/karo_to_gunshi.yaml << 'EOF'
review_request: null
EOF
...
cat > ./queue/reports/gunshi_review.yaml << 'EOF'
review:
  reviewer: gunshi
  timestamp: ""
  request_from: null
...
# 軍師ペイン（右）
tmux split-window -h -t "shogun:0"
tmux send-keys -t shogun:0.1 "cd $(pwd) && export PS1='(\[\033[1;36m\]軍師\[\033[0m\]) \[\033[1;32m\]\w\[\033[0m\]\$ ' && clear" Enter
tmux select-pane -t shogun:0.1 -T "gunshi"
tmux select-pane -t shogun:0.1 -P 'bg=#002b36'

# 将軍ペインにフォーカスを戻す
tmux select-pane -t shogun:0.0

log_success "  └─ 将軍の本陣、構築完了（将軍 + 軍師）"
echo ""
...
if [ "$SETUP_ONLY" = false ]; then
    ...
    # 将軍
    tmux send-keys -t shogun:0.0 "MAX_THINKING_TOKENS=0 claude --model opus --dangerously-skip-permissions"
    tmux send-keys -t shogun:0.0 Enter
    log_info "  └─ 将軍、召喚完了"

    # 軍師
    tmux send-keys -t shogun:0.1 "claude --dangerously-skip-permissions"
...
NINJA_EOF
    ...
    # 将軍に指示書を読み込ませる
    log_info "  └─ 将軍に指示書を伝達中..."
    tmux send-keys -t shogun:0.0 "instructions/shogun.md を読んで役割を理解せよ。"
    sleep 0.5
    tmux send-keys -t shogun:0.0 Enter
    ...
    sleep 2
    log_info "  └─ 軍師に指示書を伝達中..."
    tmux send-keys -t shogun:0.1 "instructions/gunshi.md を読んで役割を理解せよ。汝は軍師である。"
    sleep 0.5
    tmux send-keys -t shogun:0.1 Enter
...
echo "  └──────────────────────────────────────────────────────────┘"
echo ""
echo "     【shogunセッション】将軍の本陣（2ペイン）"
echo "     ┌─────────────────────────────┬─────────────────────────────┐"
echo "     │  Pane 0: 将軍 (SHOGUN)      │  Pane 1: 軍師 (GUNSHI)      │"
echo "     │  ← 総大将・プロジェクト統括  │  ← 参謀・計画レビュー       │"
echo "     └─────────────────────────────┴─────────────────────────────┘"
echo ""
